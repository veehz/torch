// This file is generated by scripts/generate_script.py from src/functions/ops.ts.j2
...
function ___left_index___tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
...
function ___right_index___tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _add_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _sub_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _mul_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _div_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _pow_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _fmod_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _maximum_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _minimum_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _log_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
function _sqrt_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
function _exp_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
function _square_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
export class Square extends UnaryFunction {
  protected _forward(a: Tensor): Tensor {
    if (a.requires_grad) {
      this.saved_tensors = [a];
    }
...
function _abs_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
function _sign_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
export class Sign extends UnaryFunction {
  protected _forward(a: Tensor): Tensor {
    if (a.requires_grad) {
      this.saved_tensors = [a];
    }
...
function _neg_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
export class Neg extends UnaryFunction {
  protected _forward(a: Tensor): Tensor {
    if (a.requires_grad) {
      this.saved_tensors = [a];
    }
...
function _reciprocal_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
export class Reciprocal extends UnaryFunction {
  protected _forward(a: Tensor): Tensor {
    if (a.requires_grad) {
      this.saved_tensors = [a];
    }
...
function _sin_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
function _cos_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
function _tan_tensor(a: Tensor, operation: TorchFunction | null = null): Tensor {
...
function _lt_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _gt_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _le_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _ge_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _eq_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
...
function _ne_tensor(a: Tensor, b: Tensor, operation: TorchFunction | null = null): Tensor {
  const broadcast_shape = _broadcast_shape(a.shape, b.shape);
  const padded_a_shape = _pad_shape(a.shape, broadcast_shape);
  const padded_b_shape = _pad_shape(b.shape, broadcast_shape);
