...
export declare function ReductionFunctionMixin(init_val: number, reduce_op: (acc: number, val: number) => number, backward_operations: (a: Tensor, restored_dz: Tensor, dim: number | number[], keepdim: boolean) => Tensor, // Simplified signature!
opName?: string | null, finalize_op?: (acc: number, count: number) => number): new () => TorchFunction;
